"""SCC-RL v5: v4 + three numerical stability fixes.

Inherits all four architectural fixes from v4:
  1. Critic Calibration   — InfoNCE on mixed goal distribution.
  2. Temporal Commitment  — CVAE subgoal held for commitment_horizon steps.
  3. Active Exploration   — CVAE injected into data collection rollouts.
  4. Improvement Filter Warm-up — filter bypassed for first cvae_warmup_steps.

Stability fixes over v4:
  A. Clamp log_var_h to [-10, 10] — prevents exp(log_var_h) overflow in KL.
  B. Mask alignment term with improvement_mask — prevents alignment from
     dominating when improvement_frac → 0 after warmup ends (which was
     causing catastrophic CVAE collapse mid-training in v4).
  C. Gradient clipping (global norm 1.0) on all optimizers.
"""
...
@dataclass
class SCCRLV5:
    """SCC-RL v5: v4 architecture with numerical stability fixes."""
    ...
    def train_fn(
        ...
        def run_eval(training_state, key):
            ...
            def step(carry, _):
                ...
                return (nstate, current_subgoal, steps_held, step_key), (current_subgoal, goal, state_obs[:, train_env.goal_indices], nstate.done)
