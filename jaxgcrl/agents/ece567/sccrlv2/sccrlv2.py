"""SCC-RL: Subgoal-Conditioned Contrastive Reinforcement Learning.

Extends the CRL agent by learning a *Geometric Subgoal Sampler*
f(z | s, g) implemented as a CVAE.  The sampler decomposes long-horizon
goal-reaching into geometrically-aligned waypoints, smoothing the
optimisation landscape of the contrastive critic.

Training loop (per gradient step):
  1. CVAE update  — ELBO on intermediate states sampled from trajectories
  2. Actor update — SAC objective with g̃ mixed from CVAE subgoal / true goal
  3. Critic update — forward InfoNCE (identical to CRL)

Inference:
  At each step, the CVAE samples a rolling waypoint z from the current
  (s, g).  The policy is conditioned on z instead of the raw far goal.
"""
...
@dataclass
class TrainingState:
    """Contains training state for the SCC-RL learner."""
    ...
    cvae_state: TrainState  # CVAEEncoder + CVAEDecoder
...
def _padded_geom_probs(gamma, is_future_mask, same_traj_mask, arrangement, seq_len):
    """Padded geometric distribution over future states for a given gamma."""
...
def flatten_batch(buffer_config, transition, sample_key):
    """Sample future goals and an intermediate subgoal from each trajectory.

    Uses separate *padded* geometric distributions for goal and subgoal sampling:
    - goal_probs uses `discounting` (longer horizon → critic/actor training)
    - subgoal_probs uses `subgoal_discounting` (shorter horizon → CVAE waypoints)

    Probability mass from the geometric tail beyond the trajectory end is
    concentrated onto the last valid future state (padded geometric).

    Goal and subgoal are sampled independently; min index → subgoal, max → goal.
    """
    ...
    # Sample goal and subgoal from their respective distributions; enforce ordering
    sample_key, subgoal_key = jax.random.split(sample_key)
    idx_a = jax.random.categorical(sample_key, jnp.log(probs_goal))   # (seq_len,)
    idx_b = jax.random.categorical(subgoal_key, jnp.log(probs_sub))   # (seq_len,)

    goal_index = jnp.maximum(idx_a, idx_b)     # farther in time → goal
    subgoal_index = jnp.minimum(idx_a, idx_b)  # closer in time  → subgoal (waypoint)

    # Standard future-state / goal (far horizon)
    future_state = jnp.take(transition.observation, goal_index[:-1], axis=0)
    future_action = jnp.take(transition.action, goal_index[:-1], axis=0)
    goal = future_state[:, goal_indices]           # (seq_len-1, goal_size)
    future_state = future_state[:, :state_size]    # (seq_len-1, state_size)

    # Intermediate state projected into goal space for CVAE training
    subgoal_obs = jnp.take(transition.observation, subgoal_index[:-1], axis=0)
    subgoal = subgoal_obs[:, goal_indices]         # (seq_len-1, goal_size)
    ...
    extras = {
        ...
        "subgoal": subgoal,  # intermediate waypoint target for CVAE
    }
...
@dataclass
class SCCRLV2:
    """Subgoal-Conditioned Contrastive RL v2 (padded geometric goal sampling)."""
    ...
    subgoal_discounting: float = 0.9  # gamma for CVAE subgoal horizon (lower → shorter waypoints)
    ...
    cvae_latent_dim: int = 64       # dimension of CVAE latent h
    cvae_beta: float = 1.0          # β-VAE coefficient on the KL term
    cvae_hidden_depth: int = 2      # depth of CVAE encoder/decoder MLPs
    cvae_alignment_coeff: float = 0.1  # λ: weight on ψ(z)^T ψ(g) alignment term
    cvae_improvement_margin: float = 0.0  # margin for subgoal improvement filter

    # Eval mixture: P(use CVAE subgoal) per step per env; 0.0 = pure true-goal
    alpha_eval: float = 0.5

    # Probability of replacing the far goal with a CVAE subgoal during actor training
    ...
    def train_fn(
        ...
        cvae_enc_params = cvae_encoder.init(
            cvae_enc_key,
            np.ones([1, state_size]),   # state
            np.ones([1, goal_size]),    # subgoal
            np.ones([1, goal_size]),    # goal
        )

        # CVAE decoder: p_ξ(z | h, s, g)
        ...
        cvae_dec_params = cvae_decoder.init(
            cvae_dec_key,
            np.ones([1, self.cvae_latent_dim]),  # h
            np.ones([1, state_size]),             # state
            np.ones([1, goal_size]),              # goal
        )
        ...
        cvae_state = TrainState.create(
            apply_fn=None,
            params={"cvae_encoder": cvae_enc_params, "cvae_decoder": cvae_dec_params},
        ...
        def deterministic_actor_step(training_state, env, env_state, key, extra_fields):
            """At inference: per-step mixture of subgoal- and true-goal-conditioned policies.

            Each env independently flips a coin: with probability alpha_eval use the
            CVAE waypoint (h=0), otherwise condition on the true goal directly.
            alpha_eval=1.0 → always subgoal; alpha_eval=0.0 → always true goal.
            """
            obs = env_state.obs                             # (num_envs, obs_size)
            state = obs[:, :state_size]                     # (num_envs, state_size)
            goal = obs[:, state_size:]                      # (num_envs, goal_size)

            # CVAE subgoal branch
            h = jnp.zeros((state.shape[0], self.cvae_latent_dim))
            subgoal = cvae_decoder.apply(
                training_state.cvae_state.params["cvae_decoder"], h, state, goal
            )
            obs_subgoal = jnp.concatenate([state, subgoal], axis=-1)
            means_subgoal, _ = actor.apply(training_state.actor_state.params, obs_subgoal)

            # True-goal branch
            means_goal, _ = actor.apply(training_state.actor_state.params, obs)

            # Per-env coin flip
            use_subgoal = jax.random.bernoulli(key, p=self.alpha_eval, shape=(state.shape[0],))
            means = jnp.where(use_subgoal[:, None], means_subgoal, means_goal)
        ...
        def actor_step(actor_state, env, env_state, key, extra_fields):
            """Stochastic step for data collection (uses true goal, same as CRL)."""
        ...
        def update_networks(carry, transitions):
            ...
            training_state, cvae_metrics = update_cvae(
                context, networks, transitions, training_state, cvae_key
            )
        ...
        def training_step(training_state, env_state, buffer_state, key):
            experience_key1, experience_key2, sampling_key, training_key = jax.random.split(
                key, 4
            )
        ...
        # Custom eval loop that threads a key so deterministic_actor_step can
        # flip a per-step, per-env coin between subgoal and true-goal policies.
        ...
        def run_eval(training_state, key):
            ...
            def step(carry, _):
                ...
                # Capture pre-step: state/goal/subgoal that actually generated this action
                obs        = env_state.obs
                state_full = obs[:, :state_size]
                goal       = obs[:, state_size:]
                h          = jnp.zeros((obs.shape[0], self.cvae_latent_dim))
                subgoal    = cvae_decoder.apply(
                    training_state.cvae_state.params["cvae_decoder"], h, state_full, goal
                )
                step_key, coin_key, next_key = jax.random.split(step_key, 3)
                new_env_state, _ = deterministic_actor_step(
                    training_state, eval_env_wrapped, env_state, coin_key,
                    extra_fields=(),
                )
                return (new_env_state, next_key), (subgoal, goal, state_full[:, :2], new_env_state.done)

            (final_state, _), (subgoals, goals, state_xys, dones) = jax.lax.scan(
                step, (eval_state, key), (), length=config.episode_length
            )
        ...
        training_walltime = 0
        logging.info("starting SCC-RL training...")
        for ne in range(config.num_evals):
            ...
            for name in ("reward", "success", "success_easy", "success_super_easy", "dist", "distance_from_origin"):
                metrics[f"eval/episode_{name}"] = float(
                    np.mean(eval_metrics.episode_metrics[name])
                )
            ...
            # Subgoal quality: distance from state position and distance to true goal
            # subgoals/goals/state_xys: (episode_length, num_eval_envs, 2)
            ...
            # Per-outcome subgoal quality scalars
            ...
            # Single failed trajectory trace every visualization_interval evals
