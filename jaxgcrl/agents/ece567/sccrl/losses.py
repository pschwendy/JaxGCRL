...
def update_cvae(config, networks, transitions, training_state, key):
    ...
    def cvae_loss_fn(cvae_params, key):
        ...
        # Reconstruction: mean squared error over goal dimensions
        recon_loss = jnp.mean(jnp.sum((z_pred - subgoal) ** 2, axis=-1))

        # KL divergence: KL(N(μ, σ²) || N(0, 1))
        #   = 0.5 * Σ (μ² + σ² − 1 − log σ²)
        kl_loss = jnp.mean(
            0.5 * jnp.sum(mu_h ** 2 + jnp.exp(log_var_h) - 1.0 - log_var_h, axis=-1)
        )
        ...
        # Freeze ψ: gradients update the CVAE only, not the critic.
        g_enc_params = jax.lax.stop_gradient(
            training_state.critic_state.params["g_encoder"]
        )
        psi_z = networks["g_encoder"].apply(g_enc_params, z_prior)          # (B, repr_dim)
        psi_g = jax.lax.stop_gradient(
            networks["g_encoder"].apply(g_enc_params, goal)                  # (B, repr_dim)
        )
