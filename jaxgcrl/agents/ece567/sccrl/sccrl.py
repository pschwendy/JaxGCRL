...
def flatten_batch(buffer_config, transition, sample_key):
    ...
    In addition to the standard CRL future-goal sampling, we draw a *second*
    future index that serves as the CVAE training target (the intermediate
    waypoint z).  We take the earlier of the two indices as the subgoal and
    the later as the goal, guaranteeing subgoal is temporally between the
...
@dataclass
class SCCRL:
    """Subgoal-Conditioned Contrastive RL agent."""
    ...
    def train_fn(
        ...
        def deterministic_actor_step(training_state, env, env_state, extra_fields):
            """At inference: use CVAE to generate a rolling waypoint subgoal.

            We decode with h = 0 (prior mean) for a deterministic subgoal
            that changes smoothly as the agent moves toward the far goal.
            """
            obs = env_state.obs                             # (num_envs, obs_size)
            state = obs[:, :state_size]                     # (num_envs, state_size)
            goal = obs[:, state_size:]                      # (num_envs, goal_size)

            # CVAE prior: h = 0  →  deterministic subgoal
            ...
            # Condition actor on the CVAE-generated waypoint
