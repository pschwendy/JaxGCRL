"""SCC-RL v8: latent-space subgoal generation.

Builds on v5 with all four architectural fixes and three stability fixes, but
replaces the goal-space CVAE with a latent-space CVAE:

  Old (v5): CVAE decoder outputs z ∈ ℝ^goal_size; actor sees [state, z]
  New (v8): CVAE decoder outputs ψ̃ ∈ ℝ^repr_dim; actor sees [state, ψ̃]

The actor input dim changes from state_size + goal_size → state_size + repr_dim.
The CVAE encoder/decoder are conditioned on ψ(g) instead of raw goal_obs.
All v5 stability fixes (log_var clamp, masked alignment, gradient clipping) are kept.
"""
...
class SCCRLV8:
    ...
    def train_fn(
        ...
        obs_size    = state_size + goal_size       # env observation size (buffer)
        actor_obs_size = state_size + self.repr_dim  # actor input: [state, ψ̃]
        ...
        actor_state = TrainState.create(
            ...
            tx=optax.chain(optax.clip_by_global_norm(1.0), optax.adam(learning_rate=self.policy_lr)),
        )
        ...
        critic_state = TrainState.create(
            ...
            tx=optax.chain(optax.clip_by_global_norm(1.0), optax.adam(learning_rate=self.critic_lr)),
        )

        # CVAE: encoder takes (state, ψ(subgoal), ψ(goal)); decoder outputs ψ̃ ∈ ℝ^repr_dim
        ...
        cvae_state = TrainState.create(
            ...
            params={
                "cvae_encoder": cvae_encoder.init(
                    ...
                    np.ones([1, self.repr_dim]),   # ψ(subgoal)
                    np.ones([1, self.repr_dim]),   # ψ(goal)
                ),
                "cvae_decoder": cvae_decoder.init(
                    cvae_dec_key,
                    np.ones([1, self.cvae_latent_dim]),
                    np.ones([1, state_size]),
                    np.ones([1, self.repr_dim]),   # ψ(goal)
                ),
            },
            tx=optax.chain(optax.clip_by_global_norm(1.0), optax.adam(learning_rate=self.cvae_lr)),
        )
        ...
        alpha_state = TrainState.create(
            ...
            tx=optax.chain(optax.clip_by_global_norm(1.0), optax.adam(learning_rate=self.alpha_lr)),
        )
        ...
        # Actor is conditioned on ψ̃ (latent subgoal) with prob alpha_subgoal,
        # otherwise on ψ(goal) — both in repr_dim space.
        def actor_step(training_state, env, env_state, key, extra_fields):
            ...
            goal_obs = obs[:, state_size:]

            # Encode goal into repr space (frozen at rollout time)
            psi_goal = jax.lax.stop_gradient(
                g_encoder.apply(
                    training_state.critic_state.params["g_encoder"], goal_obs
                )
            )
            ...
            goal_tilde = jnp.where(use_sub[:, None], psi_sub, psi_goal)

            actor_obs = jnp.concatenate([state, goal_tilde], axis=-1)
            means, log_stds = actor.apply(training_state.actor_state.params, actor_obs)
        ...
        def run_eval(training_state, key):
            ...
            init_goal_obs = init_obs[:, state_size:]

            g_enc_params = training_state.critic_state.params["g_encoder"]
            psi_init_goal = g_encoder.apply(g_enc_params, init_goal_obs)

            h0 = jnp.zeros((config.num_eval_envs, self.cvae_latent_dim))
            current_subgoal = cvae_decoder.apply(
                training_state.cvae_state.params["cvae_decoder"], h0, init_state, psi_init_goal
            )
            ...
            def step(carry, _):
                ...
                psi_goal = g_encoder.apply(g_enc_params, goal_obs)
                ...
                new_subgoal    = cvae_decoder.apply(
                    training_state.cvae_state.params["cvae_decoder"], h_new, state_obs, psi_goal
                )
                ...
                actor_obs   = jnp.concatenate([state_obs, current_subgoal], axis=-1)
                ...
                # Metrics: latent distances (ψ̃ vs ψ(goal), both in repr space)
                state_goal_obs = state_obs[:, train_env.goal_indices]
                psi_state_goal = g_encoder.apply(g_enc_params, state_goal_obs)

                return (
                    (nstate, current_subgoal, steps_held, step_key),
                    (current_subgoal, psi_goal, psi_state_goal, nstate.done),
                )

            (final_state, _, _, _), (subgoals, psi_goals, psi_state_goals, dones) = jax.lax.scan(
                step,
                (eval_state, current_subgoal, steps_held, key),
                (),
                length=config.episode_length,
            )
            return final_state, subgoals, psi_goals, psi_state_goals, dones

        eval_walltime    = 0.0
        training_walltime = 0
        logging.info("starting SCC-RL v8 training...")
        ...
        for ne in range(config.num_evals):
            ...
            t_eval = time.time()
            final_eval_state, subgoals, psi_goals, psi_state_goals, dones = run_eval(training_state, eval_key)
            ...
            # Latent-space subgoal distances
            sg  = np.array(subgoals)    # (T, num_eval_envs, repr_dim)
            pg  = np.array(psi_goals)   # (T, num_eval_envs, repr_dim)
            psg = np.array(psi_state_goals)  # (T, num_eval_envs, repr_dim)
            metrics["eval/subgoal_latent_dist_to_goal"]  = float(np.mean(np.linalg.norm(sg - pg,  axis=-1)))
            metrics["eval/subgoal_latent_dist_to_state"] = float(np.mean(np.linalg.norm(sg - psg, axis=-1)))

            logging.info("step: %d", current_step)

            do_render = ne % config.visualization_interval == 0

            # param = (actor_params, g_enc_params): encode goal before passing to actor
            def make_policy(param):
                actor_params, g_enc_params = param
                def policy(obs, rng):
                    state_  = obs[..., :state_size]
                    goal_   = obs[..., state_size:]
                    psi_g   = g_encoder.apply(g_enc_params, goal_)
                    return actor.apply(actor_params, jnp.concatenate([state_, psi_g], axis=-1))
                return policy
            ...
            progress_fn(
                ...
                metrics,
                make_policy,
                (training_state.actor_state.params, training_state.critic_state.params["g_encoder"]),
                unwrapped_env,
                do_render=do_render,
            )
            ...
            if n_failed > 0:
                metrics["eval/failed_subgoal_latent_dist_to_goal"]  = float(
                    np.mean(np.linalg.norm(sg[:, failed_mask, :] - pg[:, failed_mask, :], axis=-1))
                )
            if n_success > 0:
                metrics["eval/success_subgoal_latent_dist_to_goal"] = float(
                    np.mean(np.linalg.norm(sg[:, success_mask, :] - pg[:, success_mask, :], axis=-1))
                )
