...
Same as v5 except the improvement filter is removed entirely.
The CVAE trains on all replay buffer subgoals unconditionally.
"""
...
def update_cvae(config, networks, transitions, training_state, key):
    ...
    def cvae_loss_fn(cvae_params, key):
        ...
        recon_loss = jnp.mean(jnp.sum((z_pred - subgoal) ** 2, axis=-1))
        kl_loss    = jnp.mean(0.5 * jnp.sum(mu_h ** 2 + jnp.exp(log_var_h) - 1.0 - log_var_h, axis=-1))
        ...
        alignment = jnp.mean(jnp.sum(psi_z * psi_g, axis=-1))

        total_loss = elbo - config["cvae_alignment_coeff"] * alignment
        return total_loss, (recon_loss, kl_loss, elbo, alignment)

    (loss, (recon_loss, kl_loss, elbo, alignment)), grad = jax.value_and_grad(
    ...
    return training_state, {
        ...
        "cvae_improvement_frac": jnp.ones(()),  # always 1.0 — no filter
    }
