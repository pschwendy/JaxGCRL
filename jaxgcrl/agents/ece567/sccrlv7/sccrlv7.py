"""SCC-RL v7: random subgoal ablation.

Identical to v6 except the CVAE is removed entirely.  Subgoals are sampled
as random linear interpolations between the current state's goal-space
coordinates and the true goal: sg = s_g + t*(g - s_g), t ~ U[0,1].

This ablation tests whether learned subgoal generation (CVAE) is necessary.
"""
...
@dataclass
class SCCRLV7:
    """SCC-RL v7: random subgoal ablation (no CVAE)."""
    ...
    # How many eval steps to hold a subgoal before resampling
    ...
    def train_fn(
        ...
        goal_indices = jnp.array(train_env.goal_indices)

        def actor_step(training_state, env, env_state, key, extra_fields):
            key, interp_key, mix_key, noise_key = jax.random.split(key, 4)
            obs   = env_state.obs
            state = obs[:, :state_size]
            goal  = obs[:, state_size:]

            # Random subgoal: sg = s_g + t*(g - s_g), t ~ U[0, 1]
            state_goal_coords = state[:, goal_indices]
            t = jax.random.uniform(interp_key, (state.shape[0], 1))
            subgoal = jax.lax.stop_gradient(state_goal_coords + t * (goal - state_goal_coords))
        ...
        def run_eval(training_state, key):
            ...
            init_state_goal = init_state[:, goal_indices]
            current_subgoal = 0.5 * init_state_goal + 0.5 * init_goal
            ...
            def step(carry, _):
                ...
                t_rand          = jax.random.uniform(resample_key, (config.num_eval_envs, 1))
                state_goal_coords = state_obs[:, goal_indices]
                new_subgoal     = state_goal_coords + t_rand * (goal - state_goal_coords)
                ...
                return (nstate, current_subgoal, steps_held, step_key), (current_subgoal, goal, state_obs[:, goal_indices], nstate.done)
        ...
        eval_walltime    = 0.0
        training_walltime = 0
        logging.info("starting SCC-RL v7 (random subgoals) training...")
